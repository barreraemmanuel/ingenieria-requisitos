...
7. **Si abres la sesión con Codex CLI, confía los hooks del método UNA vez.** Un hook declarado en
   `.codex/hooks.json` no corre hasta que alguien revisa y confía su huella: escribe `/hooks` en la
   sesión y acéptalos. Si no lo haces, Codex **no los ejecuta y no te avisa** — te quedas sin el
   canario del contexto y sin el aviso de fin de turno, con la sesión con la misma pinta de siempre.
   (Con Claude Code esta puerta no existe.) El constructor y el revisor que lanza
   `docs/00-metodo/scripts/ejecucion.py` NO necesitan este paso: el lanzador ya pasa
   `--dangerously-bypass-hook-trust`, porque los hooks son los del propio método.
...
| Qué hay instalado en esta máquina · en qué se va el cupo · si un rechazo nuevo deja al agente sin salida | `python3 docs/00-metodo/scripts/doctor.py` · `scripts/coste.py` · `scripts/lint_salidas.py` (`--detalle`) |
...
12. **Evidencia, no afirmación.** Nada se da por hecho sin el output del check que lo demuestra.
    ...
    fallo repetido no se normaliza. **Los outputs largos se REFERENCIAN por ruta en `.runtime/`, no se pegan**: solo el veredicto y las líneas que lo prueban.
    **La verificación es LOCAL (ADR-035):** la suite y los lints se corren en TU máquina antes de fusionar; ese output es la evidencia. No se monta CI remoto (GitHub Actions u otro) salvo que lo pidas y quede escrito en `01-constitucion/bias.md` (`ci_remoto: sí`). El repo de código declara sus checks locales en su `AGENTS.md`.
